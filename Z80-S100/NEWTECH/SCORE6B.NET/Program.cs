...
namespace Score68B.Net
{
    class Program
    {
        static void GenerateIHEX(UInt16 startAddress, byte[] byteArray, int numBytes)
        {
            ...
            for (; i < numFullRecords; ++i)
            {
                ...
                Console.Write(string.Format(":20{0:X4}00", startAddress));
                for (int j = 0; j < 32; ++j)
                {
                ...
                Console.WriteLine(string.Format("{0:X2}", (byte)(~checksum + 1)));
                startAddress += 32;
            }
            ...
            if (remainderBytes != 0)
            {
                var checksum = (byte)remainderBytes;
                checksum += (byte)startAddress;
                checksum += (byte)(startAddress >> 8);
                Console.Write(string.Format(":{0:X2}{1:X4}00", (byte)remainderBytes, startAddress));
                            ...
                            foreach (var note in notes)
                            {
                                ...
                                switch (a)
                                {
                                    ...
                                    case 'X':
                                        ...
                                        //    Console.Write("\n");
                                        //}
                                        GenerateIHEX(256, byteArray, 3 * (v - 1) + 1);
                                        return;
                                    default:
                                        ReportError(v, trimmedNote, c);
                                        return;
